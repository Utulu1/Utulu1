<p align="center">
  <img src="anthony-utulu-banner.png" alt="Anthony Utulu — Actuarial & Business Analytics Professional" width="100%">
</p>

---

# Anthony Utulu

### Actuarial & Business Analytics Professional

**Transforming Data into Better Decisions**

---

## About Me

I specialise in applying actuarial science, statistical modelling and business analytics to solve complex business problems and support evidence-based decision-making.

My expertise spans insurance pricing, claims reserving, predictive modelling, risk analytics and business intelligence. I enjoy transforming complex datasets into clear, actionable insights that help organisations improve performance, manage risk and make informed strategic decisions.

Alongside my analytical background, my experience as a mathematics educator has strengthened my ability to communicate technical concepts clearly to both technical and non-technical audiences.

I believe the best analytics does more than explain the past—it provides confidence for better decisions about the future.

---

# Areas of Expertise

### Actuarial Analytics

- Insurance Pricing
- Claims Reserving
- Risk Analytics
- Predictive Modelling

### Business Analytics

- Business Intelligence
- KPI Reporting
- Dashboard Development
- Decision Support

### Data Analytics

- Statistical Modelling
- Data Cleaning
- Exploratory Data Analysis
- Data Visualisation

---

# Technical Skills

**Programming**

Python • SQL • R

**Analytics & BI**

Power BI • Excel

**Development**

Git • GitHub

---

# Flagship Case Studies

The repositories below represent my current flagship analytics projects and reflect the **Anthony Utulu Analytics Standard** for professional documentation, business interpretation and technical implementation.

---

## 📊 Insurance Portfolio Analytics

End-to-end insurance portfolio analytics using **SQL Server and Power BI** to evaluate profitability, claims performance, exposure and underwriting risk, and translate the findings into management decisions.

### Highlights

- 354K policy-year records analysed
- £105M premium and £74M incurred claims evaluated
- Loss Ratio, Claim Frequency and Claim Severity analysis
- Policy Type, Vehicle Age and Business Type segmentation
- SQL-based Data Quality, Cleaning and Analytics Views
- Interactive Power BI Management Dashboards
- Maintain / Monitor / Review management framework
- Executive Documentation

🔗 **Repository**

https://github.com/Utulu1/Insurance-Portfolio-Analytics

---

## 🚗 Motor Insurance Pricing Analytics

**End-to-end actuarial pricing analytics using Poisson Generalized Linear Models to model motor insurance claim frequency and support evidence-based pricing decisions.**

### Highlights

- Business-focused analytics workflow
- Real-world insurance dataset
- Exploratory Data Analysis
- Feature Engineering
- Poisson GLM
- Model Validation
- Business Insights
- Executive Documentation

🔗 Repository

https://github.com/Utulu1/Motor-Insurance-Pricing-Analytics

---

## 📊 Actuarial Claims Reserving Workflow

**End-to-end actuarial reserving analysis using deterministic and stochastic reserving techniques to estimate outstanding claims liabilities and support reserving decisions.**

### Highlights

- Chain Ladder
- Mack Chain Ladder
- Reserve Uncertainty
- Model Interpretation
- Business Reporting
- Executive Documentation

🔗 Repository

https://github.com/Utulu1/Actuarial-Claims-Reserving-Workflow

---

## 🔗 Current Development Roadmap

I am continuing to develop a portfolio of consulting-quality actuarial and business analytics case studies.

Future portfolio areas include:

- IFRS 17 Insurance Liability Modelling
- Life Insurance Pricing Analytics
- Predictive Claims Severity Modelling
- Capital & Risk Modelling
- Pension Valuation Analytics
- Executive Business Analytics Dashboards

Projects will be published once they satisfy the Anthony Utulu Analytics Standard for technical quality, business relevance and professional documentation.

---

# Professional Mission

To help organisations transform complex data into confident business decisions by combining actuarial thinking, analytics and clear communication.

---

# Connect With Me

📧 **Email**

utulu.an@gmail.com

💼 **LinkedIn**

https://www.linkedin.com/in/utulu-an

🌍 **GitHub**

https://github.com/Utulu1
